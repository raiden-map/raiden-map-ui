<template>
  <div>
    <div class="card-header bg-white">
      <div class="row">
        <div class="col h4">Raiden Network</div>
        <div class="col-7">
          <div class="row">
            <div class="col-sm">
              BTC Value
              <p class="text-secondary">{{ btcValue }}</p>
            </div>
            <div class="col-sm">
              ETH Value
              <p class="text-secondary">{{ ethValue }}</p>
            </div>
            <div class="col-sm">
              Users
            ...
            <div class="col-sm">
              Tokens
              <p class="text-secondary">{{ tokens }}</p>
            </div>
          </div>
        </div>
        <div class="col">
...
export default {
  ...
  created() {
    ...
    axios.get("RaidenDelta.json").then(resp => {
      resp.data.RAIDEN_DELTA.forEach(element => {
        if (element.key == key) {
          this.btcValue =
            element.datakey.tokenNetworksChanges[key].token.valueBtc;
          this.ethValue =
            element.datakey.tokenNetworksChanges[key].token.valueEth;
